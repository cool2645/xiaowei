...
ace.handle_side_menu = function($) {
	...
	$('#sidebar-collapse').on(ace.click_event, function() {
		...
		ace.settings.sidebar_collapsed(!$minimized);
		//@ ace-extra.js
	});
	...
	$('.nav-list').on(ace.click_event, function(e) {
		...
		if (! link_element.hasClass('dropdown-toggle')) {//it doesn't have a submenu return
			...
			if ($minimized && ace.click_event == "tap" && link_element.get(0).parentNode.parentNode == this /*.nav-list*/ )//i.e. only level-1 links
			{
		...
		var toggle_btn = $(e.target).closest('b.arrow');

		if (toggle_btn.length == 1) {
			$(sub).slideToggle(200).parent().toggleClass('open');
			return false;
		}
		//return false;
	...
	$(document).on('hide.bs.collapse show.bs.collapse', function(ev) {
		...
		$('[href*="#' + hidden_id + '"]').find('[class*="fa-"]').each(function() {
			...
			if ($icon_down) {
				...
				return false;
				//ignore other icons that match, one is enough
			}
...
ace.switch_direction = function($) {
	...
	.find('.dropdown-menu:not(.datepicker-dropdown,.colorpicker)').toggleClass('pull-right').end()
	//swap pull-left & pull-right
	.find('.pull-right:not(.dropdown-menu,blockquote,.profile-skills .pull-right)').removeClass('pull-right').addClass('tmp-rtl-pull-right').end().find('.pull-left:not(.dropdown-submenu,.profile-skills .pull-left)').removeClass('pull-left').addClass('pull-right').end().find('.tmp-rtl-pull-right').removeClass('tmp-rtl-pull-right').addClass('pull-left').end().find('.chosen-container').toggleClass('chosen-rtl').end();
	...
	if (placeholder.size() > 0) {
		...
		//draw on north-west or north-east?
