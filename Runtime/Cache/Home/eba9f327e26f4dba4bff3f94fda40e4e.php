...
<?php if(!empty($plugin["date"])): ?><link rel="stylesheet" href="/Public/Static/css/datepicker.css" /><?php endif; ?>
	...
	<?php echo W('PageHeader',array('name'=>array('消息->',$reply_name),'search'=>'N'));?>
	...
	<div class="col-xs-12">
		<div class="dialogs">
			<?php if(is_array($list)): foreach($list as $key=>$vo): ?><div class="row">
					<div class="col-xs-1">
						<div class="user">
							<img class="img-thumbnail" src="/<?php echo (get_emp_pic($vo["sender_id"])); ?>"  />
						</div>
					</div>
					<div class="col-xs-11">
						<p>
							<?php echo ($vo["sender_name"]); ?>:<?php echo ($vo["content"]); ?>
						</p>
						<p>
							<span><?php echo (to_date($vo["create_time"],'Y-m-d')); ?></span>
						</p>
						<p>
							<?php echo W('FileUpload/link',array($vo['add_file']));?>
						</p>
					</div>
				</div>
