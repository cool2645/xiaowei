	...
	<div class="ul_table">
		...
		<?php if(empty($list)): ?>没找到数据
			...
			<?php if(is_array($list)): foreach($list as $key=>$vo): ?><div class="row">
					<div class="col-xs-1">
						<div class="user">
							<?php if(($vo["sender_id"]) == $owner_id): ?><img class="img-thumbnail" src="/<?php echo (get_emp_pic($vo["receiver_id"])); ?>" /><?php endif; ?>
							<?php if(($vo["receiver_id"]) == $owner_id): ?><img class="img-thumbnail" src="/<?php echo (get_emp_pic($vo["sender_id"])); ?>" /><?php endif; ?>
						</div>
					</div>
					<div class="col-xs-11">
						<p>
							<?php if(($vo["sender_id"]) == $owner_id): echo ($vo["receiver_name"]); ?>:<?php echo ($vo["content"]); endif; ?>
							<?php if(($vo["receiver_id"]) == $owner_id): echo ($vo["sender_name"]); ?>:<?php echo ($vo["content"]); endif; ?>
						</p>
						<p>
							<span><?php echo (to_date($vo["create_time"],'Y-m-d')); ?></span>
							<span class="pull-right">
								<?php if(($vo["sender_id"]) == $owner_id): ?><a href="<?php echo U('read','reply_id='.$vo['receiver_id']);?>">共<?php echo ($vo["count"]); ?>条对话</a>
									...
									<a onclick="del_message(<?php echo ($vo["receiver_id"]); ?>)">删除</a><?php endif; ?>
								<?php if(($vo["receiver_id"]) == $owner_id): ?><a href="<?php echo U('read','reply_id='.$vo['sender_id']);?>">共<?php echo ($vo["count"]); ?>条对话</a>
									<a onclick="reply_message(<?php echo ($vo["sender_id"]); ?>)">回复</a>
									<a onclick="del_message<?php echo ($vo["sender_id"]); ?>">删除</a><?php endif; ?></span>
						</p>
					</div>
				</div>
