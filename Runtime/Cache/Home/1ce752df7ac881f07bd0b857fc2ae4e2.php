			...
			<?php if(empty($list)): ?><ul>
					...
					<form method="post" action="" name="form_data" id="form_data">
						<?php if(is_array($list)): foreach($list as $key=>$vo): ?><li class="tbody  <?php if((!in_array($vo['id'],$readed_id))&&($vo['create_time']>=time()-3600*30*24)) echo("bold") ?>" >
								...
								<div class="pull-right">
									<span class="col-10 "> <?php echo (to_date($vo["create_time"],'Y-m-d')); ?> </span>
...
<script type="text/javascript">
	...
	<script type="text/javascript">
		function add() {
			window.open("<?php echo U('add','fid='.$fid);?>", "_self");
		}

		function del() {
			ui_confirm('确定要删除吗?', function() {
				var vars = $("#form_data").serialize();
				sendAjax("<?php echo U('del');?>", vars);
		...
		function move_to(val) {
			var vars = $("#form_data").serialize();
			sendAjax("<?php echo U('move_to');?>", 'val=' + val + '&' + vars);
			$("input[name='id[]']:checkbox:checked").each(function() {
				location.reload();
			});
		}
